...
export const DEFAULT_LIGHT_MODEL = 'SmolLM2-360M-Instruct-q4f16_1-MLC';
...
export class WebGPUDetector {
  ...
  public async detectWebGPU(): Promise<WebGPUReport> {
    ...
    try {
      ...
      const recommendedModelId = isFullCapability ? DEFAULT_FULL_MODEL : DEFAULT_LIGHT_MODEL;

      logger.info(`WebGPU active: ${adapterName} (${vendor}), maxBuffer: ${maxBufferSizeMB}MB, tier: ${tier}`);
      ...
      return {
        ...
        maxBufferSizeMB,
        recommendedModelId,
        notes: `WebGPU acceleration active. Recommended model: ${recommendedModelId}.`,
      };
    } catch (err) {
